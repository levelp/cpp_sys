...
// Test 26: Long type whole number output (covers line 75 for Rational<long>)
TEST_F(RationalTest, LongTypeWholeNumber) {
    Rational<long> r(10L, 2L);  // Should normalize to 5/1
    std::ostringstream oss;
    oss << r;
    EXPECT_EQ(oss.str(), "5");
}

// Test 27: Long type division resulting in whole number
TEST_F(RationalTest, LongTypeDivisionWholeNumber) {
    Rational<long> a(6L, 2L), b(3L, 2L);  // a = 3, b = 1.5
    Rational<long> c = a / b;  // 3 / 1.5 = 6/2 / 3/2 = 12/6 = 2
    std::ostringstream oss;
    oss << c;
    EXPECT_EQ(oss.str(), "2");
}
